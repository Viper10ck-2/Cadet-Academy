...
<body class="h-full bg-[#0F172A] text-white font-sans antialiased flex flex-col" x-data>
...
<nav class="bottom-nav">
    <div class="max-w-lg mx-auto flex items-center justify-around px-4">
        <a href="{{ route('absen.dashboard', ['absen' => 1]) }}" class="flex flex-col items-center gap-1 px-3 py-1 {{ request()->routeIs('absen.dashboard') ? 'text-[#D4A853]' : 'text-gray-400' }}">
            <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z"/></svg>
            <span class="text-[10px] font-medium">Home</span>
        </a>
        <a href="{{ route('absen.history', ['absen' => 1]) }}" class="flex flex-col items-center gap-1 px-3 py-1 {{ request()->routeIs('absen.history') ? 'text-[#D4A853]' : 'text-gray-400' }}">
        ...
        <a href="{{ route('absen.profile', ['absen' => 1]) }}" class="flex flex-col items-center gap-1 px-3 py-1 {{ request()->routeIs('absen.profile') ? 'text-[#D4A853]' : 'text-gray-400' }}">
...
if ('serviceWorker' in navigator && window.isSecureContext) {
    navigator.serviceWorker.register('/absen-sw.js').catch(() => {});
}
</script>
</body>
</html>
