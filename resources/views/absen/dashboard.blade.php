...
<div class="flex-1 navy-gradient" x-data="absenApp()">
    ...
    <div class="px-5 pt-8 pb-6 text-center">
        ...
        <p class="text-gray-400 text-xs mt-0.5">{{ $user->nip_nis ?? 'N/A' }}</p>
        @foreach($myClasses as $c)<span class="inline-block mt-1 px-2 py-0.5 bg-[#D4A853]/20 text-[#D4A853] text-[10px] rounded-full">{{ $c->name }}</span>@endforeach
    </div>

    {{-- Info Card --}}
    <div class="mx-5 bg-white/5 backdrop-blur rounded-2xl p-5 border border-white/10 mb-4">
        <div class="grid grid-cols-2 gap-4 text-center">
            <div><p class="text-gray-400 text-[10px] uppercase tracking-wider">Hari</p><p class="text-white font-semibold text-sm mt-0.5">{{ now()->locale('id')->translatedFormat('l') }}</p></div>
            <div><p class="text-gray-400 text-[10px] uppercase tracking-wider">Tanggal</p><p class="text-white font-semibold text-sm mt-0.5">{{ now()->format('d F Y') }}</p></div>
            ...
            <div>
                ...
                <p class="text-gray-500 text-[10px]">{{ $checkedIn->created_at->format('H:i') }}</p>
                @else
...
document.addEventListener('alpine:init',()=>{Alpine.data('absenApp',()=>({
    ...
    async doAbsen(type){
        ...
        try{
            ...
            const res=await fetch('{{route("absen.store")}}',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':'{{csrf_token()}}','Accept':'application/json'},body:JSON.stringify({type,latitude:pos.coords.latitude,longitude:pos.coords.longitude,photo})});
